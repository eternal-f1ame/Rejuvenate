...
def training_loop(
    ...
    val_set = dnnlib.util.construct_class_by_name(
        **val_set_kwargs)  # subclass of training.dataset.Dataset
    ...
    if rank == 0:
        ...
        # G
        img_in = torch.empty([batch_gpu, training_set.num_channels,
                             training_set.resolution, training_set.resolution], device=device)
        mask_in = torch.empty(
            [batch_gpu, 1, training_set.resolution, training_set.resolution], device=device)
        img = misc.print_module_summary(G, [img_in, mask_in, z, c])
        # D
        img_stg1 = torch.empty(
            [batch_gpu, 3, training_set.resolution, training_set.resolution], device=device)
    ...
    for name, module, opt_kwargs, reg_interval in [('G', G, G_opt_kwargs, G_reg_interval), ('D', D, D_opt_kwargs, D_reg_interval)]:
        if reg_interval is None:
            ...
            if 'lrt' in opt_kwargs:
                ...
                optim_params = [{'params': base_params}, {
    ...
    if rank == 0:
        ...
        grid_size, images, masks, labels = setup_snapshot_image_grid(
            training_set=training_set)
        save_image_grid(images, os.path.join(run_dir, 'reals.png'),
                        drange=[0, 255], grid_size=grid_size)
        # adaptation to inpainting config
        save_image_grid(masks, os.path.join(run_dir, 'masks.png'),
        ...
        # adaptation to inpainting config
        grid_img = (torch.from_numpy(images).to(device) /
                    127.5 - 1).split(batch_gpu)  # [-1, 1]
        grid_mask = torch.from_numpy(masks).to(
            device).split(batch_gpu)  # {0, 1}
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            phase_real_img, phase_mask, phase_real_c = next(
                training_set_iterator)
            phase_real_img = (phase_real_img.to(device).to(
                torch.float32) / 127.5 - 1).split(batch_gpu)
            # adaptation to inpainting config
            phase_mask = phase_mask.to(device).to(
                torch.float32).split(batch_gpu)
            # --------------------
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            for round_idx, (real_img, mask, real_c, gen_z, gen_c) in enumerate(zip(phase_real_img, phase_mask, phase_real_c, phase_gen_z, phase_gen_c)):
                ...
                gain = phase.interval
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, mask=mask,
        ...
        if (network_snapshot_ticks is not None) and (done or cur_tick % network_snapshot_ticks == 0):
            snapshot_data = dict(training_set_kwargs=dict(
                training_set_kwargs), val_set_kwargs=dict(val_set_kwargs))
            for name, module in [('G', G), ('D', D), ('G_ema', G_ema), ('augment_pipe', augment_pipe)]:
                if module is not None:
                    if num_gpus > 1:
                        misc.check_ddp_consistency(module, ignore_regex=[
                                                   r'.*\.w_avg', r'.*\.relative_position_index', r'.*\.avg_weight', r'.*\.attn_mask', r'.*\.resample_filter'])
